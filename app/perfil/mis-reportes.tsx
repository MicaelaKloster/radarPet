...
export default function MisReportes() {
  ...
  return (
    <ThemedView style={styles.container}>
      <ThemedText type="title" style={styles.title}>Mis Reportes</ThemedText>
          ...
          reportes.map((reporte) => (
            <View key={reporte.id} style={styles.reporteCard}>
              <View style={styles.reporteHeader}>
                <IconSymbol
                  size={24}
                  name={reporte.tipo === 'perdida' ? 'exclamationmark.circle.fill' : 'checkmark.circle.fill'}
                  color={reporte.tipo === 'perdida' ? '#FF6B6B' : '#4ECDC4'}
                />
                <View style={styles.reporteInfo}>
                  <ThemedText style={styles.reporteTitulo}>{reporte.titulo}</ThemedText>
              ...
              {reporte.estado !== 'resuelto' && reporte.estado !== 'cerrado' && (
                <TouchableOpacity
                  style={styles.botonResolver}
                  onPress={() => marcarComoResuelto(reporte.id, reporte.titulo)}
                >
                  <ThemedText style={styles.textoBotonResolver}>Marcar como resuelto</ThemedText>
                </TouchableOpacity>
              )}
            </View>
          ))
