...
export default function ReportFoundScreen() {
  ...
  const elegirFoto = async () => {
    try {
      ...
      if (result.canceled) {
        console.log("[Reporte Encontradas] Selección cancelada por el usuario");
        return;
      }

      if (!result.assets || result.assets.length === 0) {
        console.log("[Reporte Encontradas] No se seleccionó ninguna imagen");
        return;
      }

      const image = result.assets[0];
      console.log("[Reporte Encontradas] Imagen seleccionada:", {
        uri: image.uri,
        width: image.width,
        height: image.height,
        type: image.type,
      ...
      console.log("[Reporte Encontradas] Imagen lista para subir:", {
        uri: processedImage.uri,
        width: processedImage.width,
        height: processedImage.height,
      });
      ...
      console.error(
        "[Reporte Encontradas] Error al seleccionar la imagen:",
        error
      );
      Alert.alert(
        "Error",
  ...
  const subirFotoSiExiste = async (reporteId: string): Promise<boolean> => {
    ...
    try {
      ...
      console.log("[Reporte Encontradas] preparar subida:", {
        reporteId,
        extRaw,
        ext,
        path,
      ...
      if (Platform.OS === "web") {
        ...
        console.log("[Reporte Encontradas] subiendo a storage (web)...", {
          contentType,
          size: blob.size,
        });
      ...
      console.log(
        "[Reporte Encontradas] Foto subida en:",
        path,
        "URL:",
        pub?.publicUrl
      );
  ...
  const publicar = async () => {
    ...
    try {
      ...
      if (sessionError)
        console.warn("[Reporte Encontradas] getSession error:", sessionError);
      ...
      if (parsed) {
        coords = parsed;
        console.log(
          "[Reporte Encontradas] usando coordenadas ingresadas:",
          coords
        );
        ...
        try {
          ...
          console.log(
            "[Reporte Encontradas] usando ubicación del GPS:",
            coords
          );
        } catch (e) {
          console.warn(
            "[Reporte Encontradas] getCurrentPositionAsync falló, probando lastKnown:",
            e
          );
          try {
            ...
            if (last?.coords) {
              ...
              console.log(
                "[Reporte Encontradas] usando última ubicación conocida:",
                coords
              );
            }
          } catch (e2) {
            console.warn(
              "[Reporte Encontradas] getLastKnownPositionAsync falló:",
              e2
            );
          }
        }
      ...
      if (!coords)
        throw new Error(
          "No se pudo determinar la ubicación. Seleccioná un punto en el mapa."
        );
        ...
        supabase
          ...
          .insert({
            ...
            descripcion:
              ...
              `Mascota encontrada por usuario`,
            visto_por_ultima_vez: formData.fechaHoraHallazgo.trim()
              ? validarFecha(formData.fechaHoraHallazgo)
      ...
      console.log(
        "[Reporte Encontradas] estado foto antes de subir:",
        !!foto,
        foto?.uri
      );
      if (foto) {
      ...
      if (Platform.OS === "web")
        ...
        Alert.alert(
          "¡Gracias!",
          "Tu reporte de mascota encontrada fue publicado."
        );
      ...
      >
        ...
        <View style={styles.formSection}>
          ...
          <View style={styles.formField}>
            ...
            <TextInput
              ...
              onChangeText={(t) =>
                setFormData((p) => ({ ...p, fechaHoraHallazgo: t }))
              }
              editable={!loading.publicando}
            />
          </View>
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity
            style={[
              styles.submitButton,
              (estaCargando || !formData.especieId) &&
                styles.submitButtonDisabled,
            ]}
            onPress={() => {
              console.log("[Reporte Encontradas] Botón Publicar presionado", {
                estaCargando,
