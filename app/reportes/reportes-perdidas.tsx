...
async function withTimeout<T = any>(
  ...
  const timeoutPromise = new Promise<never>((_, reject) => {
    timeoutId = setTimeout(
      () => reject(new Error(`Timeout (${label}) después de ${ms}ms`)),
      ms
    );
  });
  try {
...
function validarFecha(fechaString: string): string {
  ...
  if (!parsed || isNaN(parsed.getTime())) {
    throw new Error(
      "Formato de fecha inválido. Usa YYYY-MM-DD HH:MM o DD/MM/YYYY"
    );
  }
...
export default function ReportesPerdidasScreen() {
  ...
  const [mascotasRegistradas, setMascotasRegistradas] = useState<any[]>([]);
  ...
  useEffect(() => {
    const cargarMascotasUsuario = async () => {
      try {
        const {
          data: { user },
  ...
  const seleccionarMascota = (mascotaId: string) => {
    ...
    setFormData({
      ...
      especieId: mascota.especie_id || null,
      raza: mascota.raza || "",
      tamanioId: mascota.tamanio_id || null,
      sexoId: mascota.sexo_id || null,
      color: mascota.color || "",
      seniasParticulares: mascota.senias_particulares || "",
      ultimaUbicacion: formData.ultimaUbicacion,
      fechaHoraPerdida: formData.fechaHoraPerdida,
      descripcionUbicacion: formData.descripcionUbicacion,
  ...
  useEffect(() => {
    const cargarCatalogos = async () => {
      try {
        ...
        const [especiesRes, tamaniosRes, sexosRes] = resultados.map((result) =>
  ...
  const solicitarMiUbicacion = async () => {
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          "Permiso denegado",
          "No se puede obtener tu ubicación sin permisos."
        );
        return;
      }
      ...
      setFormData((prev) => ({
        ...prev,
        ultimaUbicacion: `${coords.latitude.toFixed(
          5
  ...
  const elegirFoto = async () => {
    try {
      ...
      console.log("[Reportes Perdidas] asset seleccionado:", {
        uri: asset.uri,
        width: asset.width,
        height: asset.height,
      ...
      if (!validarTipoImagen(asset.uri)) {
        console.warn(
          "[Reportes Perdidas] validación de imagen falló, pero se permite por configuración"
        );
      }
      ...
      console.log("[Reportes Perdidas] foto seleccionada:", {
        uri: manipResult.uri,
        w: manipResult.width,
  ...
  const publicar = async () => {
    ...
    if (errores.length) {
      ...
      if (Platform.OS === "web")
        (globalThis as any).alert?.(errores.join("\n"));
    ...
    try {
      ...
      if (sessionError)
        console.warn("[Reportes Perdidas] getSession error:", sessionError);
      ...
      if (!user)
        throw new Error(
          "No estás autenticado. Iniciá sesión e intentá nuevamente."
        );
      ...
      if (mascotaSeleccionada) {
        // Si seleccionó una mascota registrada, usar ese ID
        mascotaId = mascotaSeleccionada;
        console.log(
          "[Reportes Perdidas] usando mascota registrada:",
          mascotaId
        );
      } else {
      ...
      if (!tipoRes?.data?.id || !estadoRes?.data?.id)
        throw new Error("Faltan catálogos requeridos");
      ...
      if (parsed) {
        coords = parsed;
        console.log(
          "[Reportes Perdidas] usando coordenadas ingresadas:",
          coords
        );
      ...
      const { data: nuevoReporte, error: reporteError } = await withTimeout(
        supabase
          ...
          .insert({
            ...
            descripcion:
              formData.seniasParticulares.trim() ||
              `Mascota perdida reportada por usuario`,
            visto_por_ultima_vez: formData.fechaHoraPerdida.trim()
      ...
      console.log(
        "[Reportes Perdidas] estado foto antes de subir:",
        !!foto,
        foto?.uri
      );
      const ok = await withTimeout(
        subirFotoSiExiste(nuevoReporte.id),
        30000,
        "subir foto"
      );
      ...
      if (Platform.OS === "web")
        (globalThis as any).alert?.("¡Reporte publicado!");
      else
        Alert.alert(
          "¡Gracias!",
          "Tu reporte de mascota perdida fue publicado."
        );
        ...
        <View style={[styles.formSection, { paddingHorizontal: 20 }]}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <TouchableOpacity
              style={[
                styles.mascotaCard,
                !mascotaSeleccionada && styles.mascotaCardSelected,
              ]}
              onPress={() => {
                setMascotaSeleccionada(null);
                setFormData((prev) => ({
            ...
            >
              ...
              <ThemedText style={styles.mascotaCardText}>Nueva</ThemedText>
            </TouchableOpacity>
              ...
              <TouchableOpacity
                ...
                style={[
                  ...
                  mascotaSeleccionada === mascota.id &&
                    styles.mascotaCardSelected,
                ]}
                onPress={() => seleccionarMascota(mascota.id)}
              >
                {mascota.foto_principal_url ? (
                  <Image
                    source={{ uri: mascota.foto_principal_url }}
                    style={styles.mascotaCardImage}
                  />
                ) : (
                  <View style={styles.mascotaCardPlaceholder}>
                    <IconSymbol size={30} name="pawprint.fill" color="#999" />
                  </View>
                )}
                <ThemedText style={styles.mascotaCardText} numberOfLines={1}>
                  {mascota.nombre}
                </ThemedText>
              </TouchableOpacity>
            ))}
      ...
      >
        ...
        <View style={styles.formSection}>
          ...
          <View style={styles.formField}>
            ...
            <TouchableOpacity
              ...
              disabled={loading.publicando || loading.obteniendoUbicacion}
            >
          ...
          <View style={styles.formField}>
            ...
            <TextInput
              ...
              onChangeText={(t) =>
                setFormData((p) => ({ ...p, descripcionUbicacion: t }))
              }
              editable={!loading.publicando}
              maxLength={300}
            />
          ...
          <View style={styles.formField}>
            <Text
              style={[styles.fieldLabel, { color: isDark ? "#fff" : "#000" }]}
            >
              Fecha/hora de la pérdida (opcional)
            </Text>
            <TextInput
              style={styles.textInput}
              ...
              onChangeText={(t) =>
                setFormData((p) => ({ ...p, fechaHoraPerdida: t }))
              }
              editable={!loading.publicando}
            />
          </View>
          ...
          {foto ? (
            ...
            >
              ...
              <ThemedText style={styles.photoText}>
                Agregar foto de la mascota (obligatorio)
              </ThemedText>
        ...
        <View style={styles.buttonContainer}>
          <TouchableOpacity
            style={[
              styles.submitButton,
              (estaCargando || !formData.especieId || !foto) &&
                styles.submitButtonDisabled,
            ]}
            onPress={() => {
              console.log("[Reportes Perdidas] Botón Publicar presionado", {
                estaCargando,
